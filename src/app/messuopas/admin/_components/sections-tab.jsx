...
export default function SectionsTab() {
    ...
    // Create subsection
    const onCreateSubsection = async (data) => {
        try {
            setIsSubmitting(true);
            ...
            const updatedSubsections = [...selectedSectionForSubsection.initialSubsections, newSubsection.$id];
            await updateDocument('main_db', 'initial_sections', selectedSectionForSubsection.$id, {
                initialSubsections: updatedSubsections
            });
    ...
    // Update subsection
    const onUpdateSubsection = async (data) => {
        try {
            setIsSubmitting(true);
            const { error } = await updateDocument('main_db', 'initial_subsections', editingSubsection.$id, {
                title: data.title,
                html: data.html,
                order: data.order
            });
